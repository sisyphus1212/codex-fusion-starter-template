...
为提升跨项目复用能力，融合模板新增 `scaffold/governance/`：

1. Governance Core：`state/`、`checklists/`、`templates/` 固定治理契约。
2. Project Profile：`PROJECT_PROFILE.yaml` 用参数描述项目约束与验证策略。
3. Domain Adapter：`scripts/*.commands` 注入项目命令，实现“换项目不换治理框架”。

新增后收益：

- 可以把特定项目手册抽象成参数和命令，而不是复制整份项目特定提示词。
- 进度真值、恢复入口、任务拆分、证据归档、固定汇报格式都有统一文件承载。
- 不同语言项目（C/C++/Python/Rust）可共享同一治理骨架。
