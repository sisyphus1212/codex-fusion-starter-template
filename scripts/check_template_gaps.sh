...
while IFS= read -r rel; do
  ...
  total_structural=$((total_structural + 1))
...
# Check governance abstraction layer presence and executability.
for f in \
  "governance/README.md" \
  "governance/PROJECT_PROFILE.yaml" \
  "governance/state/TASKBOARD_MASTER.md" \
  "governance/state/RECOVERY_RUNBOOK.md" \
  "governance/state/BRANCH_AND_COMMIT_POLICY.md" \
  "governance/state/TASK_SPLIT_RULES.md" \
  "governance/checklists/MIN_REGRESSION.md" \
  "governance/checklists/EVIDENCE_REQUIRED.md" \
  "governance/templates/STATUS_REPORT.md" \
...

{
  ...
  if [ ${#missing_structural[@]} -eq 0 ]; then
    echo "- 结果: 无缺失（${total_structural}/${total_structural} 文件齐全）。"
  else
    present=$((total_structural - ${#missing_structural[@]}))
    echo "- 结果: 发现 ${#missing_structural[@]} 个缺失文件（${present}/${total_structural}）。"
  ...
  echo "## 3) 关键融合能力检查"
  if [ ${#checks[@]} -eq 0 ]; then
    echo "- 结果: 融合关键能力完整（AGENTS 双来源 + Python Codex runtime + Codex schema + Codex Action + Governance 抽象层）。"
  else
